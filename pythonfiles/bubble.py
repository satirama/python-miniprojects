...
def keydown(key):
    ...
    if key == simplegui.KEY_MAP['space']:
        if bubble_stuck:
            vel_constant = 4
            vel_o = angle_to_vector(firing_angle)
            vel = [vel_o[0] * vel_constant, vel_o[1] * vel_constant]
...
def draw(canvas):
    ...
    # draw firing line
    upper_x = FIRING_POSITION[0] + angle_to_vector(firing_angle)[0] * FIRING_LINE_LENGTH
    upper_y = FIRING_POSITION[1] - angle_to_vector(firing_angle)[1] * FIRING_LINE_LENGTH
    canvas.draw_line(FIRING_POSITION, [upper_x, upper_y], 3, "White")
