...
def new_game():
    ...
    up_tiles = [Tile(tile_numbers[i], False, [TILE_WIDTH * i, TILE_HEIGHT]) for i in range(DISTINCT_TILES)]
    down_tiles = [Tile(tile_numbers[i], False, [TILE_WIDTH * range(DISTINCT_TILES, DISTINCT_TILES*2).index(i) , TILE_HEIGHT * 2]) for i in range(DISTINCT_TILES, DISTINCT_TILES*2)]
    my_tiles = up_tiles + down_tiles
